...
class TestGameRecording(unittest.TestCase):
    def setUp(self):
        ...
        self.recorder.move_log_file = "test_moves.txt"  # Use test file
    
    def tearDown(self):
        import os
        if os.path.exists(self.recorder.move_log_file):
            os.remove(self.recorder.move_log_file)
    
    # AC 20.1 Recording Game
    def test_recording_enabled(self):
        # Start recording
        self.recorder.start_recording("Simple", "Human", "Computer", 3)
        
        # Make some moves
        self.recorder.record_move("Red", 0, 0, "S")
        self.recorder.record_move("Blue", 1, 1, "O")
        
        # End recording
        self.recorder.end_recording("Red", 1, 0)
        
        # Verify file was created and contains moves
        with open(self.recorder.move_log_file, 'r') as f:
            content = f.read()
            self.assertIn("New Game Started", content)
            self.assertIn("1. Red: S at (0,0)", content)
            self.assertIn("2. Blue: O at (1,1)", content)
            self.assertIn("Winner: Red", content)
    
    # AC 20.2 Not Recording Game
    def test_recording_disabled(self):
        ...
        import os
        self.assertFalse(os.path.exists(self.recorder.move_log_file))
...
class TestGameReplay(unittest.TestCase):
    def setUp(self):
        ...
        self.recorder = GameRecorder()
        self.logic = GLogic()
        self.recorder.move_log_file = "test_moves.txt"
        
        # Setup a recorded game
        self.recorder.start_recording("Simple", "Human", "Computer", 3)
        self.recorder.record_move("Red", 0, 0, "S")
        self.recorder.record_move("Blue", 1, 1, "O")
        self.recorder.end_recording("Red", 1, 0)
    ...
    # AC 21.1 Replaying a recorded game
    def test_replay_recorded_game(self):
        # Get the moves from recorder
        moves = []
        with open(self.recorder.move_log_file, 'r') as f:
            for line in f:
                if line.strip() and line[0].isdigit():
                    parts = line.split()
                    move_num = int(parts[0].strip('.'))
                    player = parts[1].strip(':')
                    letter = parts[2]
                    coords = parts[4].strip('()').split(',')
                    row, col = int(coords[0]), int(coords[1])
                    moves.append((player, row, col, letter))
        
        # Verify we got the moves
        self.assertEqual(len(moves), 2)
        self.assertEqual(moves[0][0], "Red")
        self.assertEqual(moves[1][0], "Blue")
    
    # AC 21.2 Replaying a non-recorded game
    def test_replay_non_recorded_game(self):
        # Clear the recorder
        self.recorder = GameRecorder()
        self.recorder.move_log_file = "test_moves.txt"
        
        # Verify no moves exist
        import os
        if os.path.exists(self.recorder.move_log_file):
            os.remove(self.recorder.move_log_file)
        
        # Try to get moves (should raise error)
